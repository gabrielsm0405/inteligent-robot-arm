# -*- coding: utf-8 -*-
"""automation_projetc_identification_random_forest.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/14mg_PP9AYmuyERte0IOkMzBq_EfYt_gG
"""
...
dump(clf, 'random_forest_identification.joblib')

model = load('random_forest_identification.joblib')

y_predicy_ga = model.predict(X_test)
mean_squared_error(y_test,y_predicy_ga)

window = (10000, 15000)
n = list(range(window[0], window[1]))
x = X.to_numpy()[window[0]:window[1]]
pgp = model.predict(x)

gx = y.iloc[:, 0].to_numpy()[window[0]:window[1]]
pgx = pgp[:, 0]
plt.plot(n, gx, color='r')
plt.plot(n, pgx, color='g')
plt.show()

gy = y.iloc[:, 1].to_numpy()[window[0]:window[1]]
pgy = pgp[:, 1]
plt.plot(n, gy, color='r')
plt.plot(n, pgy, color='g')
plt.show()

gz = y.iloc[:, 2].to_numpy()[window[0]:window[1]]
pgz = pgp[:, 2]
plt.plot(n, gz, color='r')
plt.plot(n, pgz, color='g')
plt.show()